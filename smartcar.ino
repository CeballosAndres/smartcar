...
#define ECHO_PIN 12 // D6 with a jump
...
void setup() {
  ...
  servoMotor.attach(SERVO);  // attach servoMotor on pin 3 to servoMotor object
  Serial.begin(115200);
  pinMode(MDA, OUTPUT);
  pinMode(MDB, OUTPUT);
  pinMode(MPA, OUTPUT);
  ...
  stop();
  servoMotor.write(90);  //posicion inicial en el centro
  delay(5000); 
} 
...
void conducir()
{
  ...
  ******************************************************************/
  ...
  if((out1+out2)==1){ // si es giro, necesita doble fuerza los motores
    carSpeed = SPEED + 100;
  }
